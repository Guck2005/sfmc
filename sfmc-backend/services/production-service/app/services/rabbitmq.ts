...
// Defining DomainEvent directly here instead of shared-types if not present
...
type RabbitMQPublishHook = (event: DomainEvent) => Promise<void> | void

declare global {
  // eslint-disable-next-line no-var
  var __sfmcRabbitMQPublishHook: RabbitMQPublishHook | undefined
}
...
export async function publishEvent(event: DomainEvent): Promise<void> {
  ...
  const ch = getChannel()
  ch.publish(EXCHANGE_NAME, event.type, Buffer.from(JSON.stringify(event)), {
