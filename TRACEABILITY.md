...
Ce document relie les **besoins fonctionnels (BF)** des sections **1 → 7** et les **cas d’utilisation CU-01 / CU-02 / CU-03** aux services, événements ou endpoints principaux, aux tests automatisés et aux écrans du back-office.
...
| **1.2** Suivi temps réel avancement OF | reporting-service | GraphQL subscription `productionOrderProgress` (schéma reporting) | `reporting-service/tests/unit/reporting_kpis.spec.ts` | — | Health `reporting=ok` | `Production.tsx`, `Dashboard.tsx` |
...
| **2.1** Comptes clients (CRUD / désactivation) | user-service | `GET/POST/PATCH /api/v1/users` | `user-service/tests/unit/user_policy.spec.ts` | — | Health `user=ok` | `Users.tsx` |
...
| **2.2** Transitions manuelles de statut (opérateur) | order-service | `PATCH /api/v1/orders/:id/status` | `order-service/tests/unit/orders_policy.spec.ts` | — | — | `OrderDetail.tsx` |
...
| **2.3** Paiement + statuts facture + PDF | billing-service | `POST /api/v1/payments`, export PDF facture | `billing-service/tests/unit/pdf_invoice.spec.ts` | — | — | `Billing.tsx` |
...
| **2.4** Livraison `SHIPPED` → `DELIVERED` + notification expédition | order-service, notification-service | `order.shipped` (email client) | `notification-service/tests/unit/dispatcher.spec.ts` | — | — | `OrderDetail.tsx`, `e2e/order-flow.spec.ts` |
...
| **3.2** Multi-entrepôts + stocks par entrepôt | inventory-service | `GET/POST /api/v1/warehouses`, stocks filtrés | — | — | Health `inventory=ok` | `Inventory.tsx` |
...
| **4** Catalogue produits (catégories, prix, actif/inactif, filtres) | product-service | `GET/POST/PATCH /api/v1/products` | `product-service/tests/unit/product_model.spec.ts` | — | Health `product=ok` | `Products.tsx` |
...
| **6** Matrice notifications **e-mail** (commande, production, stock, facture) | notification-service | Consommation événements → Brevo SMTP | `notification-service/tests/functional/notification_matrix.spec.ts`, `tests/unit/dispatcher.spec.ts` | — | CU-01 #7 `ORDER_VALIDATED` ; CU-02 #5 `PRODUCTION_COMPLETED` | `Notifications.tsx` |
| **7** Tableau de bord KPI + agrégats temps réel (CQRS) | reporting-service | `GET /api/v1/reports/kpis`, GraphQL `kpiUpdated` | `reporting-service/tests/unit/reporting_kpis.spec.ts` | — | Health `reporting=ok` + flux KPI via events | `Dashboard.tsx`, `e2e/dashboard-realtime.spec.ts` |
| **7** Rapports par période + export CSV | reporting-service | Paramètres période + export manuel CSV | `reporting-service/tests/unit/date_range.spec.ts`, `tests/unit/csv_export.spec.ts` | — | — | `Reports.tsx` |
...
| **CU-03** Mouvement stock → sous-seuil → `inventory.critical` → e-mail | inventory-service, notification-service | `PUT …/stocks/:id/threshold`, `POST …/movements` | `inventory-service/tests/unit/stock_rules.spec.ts` | `inventory-service/tests/integration/cu03_critical.spec.ts` | CU-03 #1–#3 | `Inventory.tsx`, `Notifications.tsx` |
...
## Definition of Done (v1.0.0-rc1) — contrôle du 2026-04-20
...
*Document généré dans le cadre du **Lot 9** — traçabilité et définition des finitions (DoD) v1.0.0-rc1.*
